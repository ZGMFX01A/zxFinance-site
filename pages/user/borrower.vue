...
export default {
  ...
  methods: {
    ...
    onUploadSuccess(response,file,type){
      if(response.code===0){
        //成功,填充borrower.borrerAttachList
    ...
    onUploadRemove(file,fileList){
      ...
      //调用远程文件删除接口
      this.$axios.$delete('/api/oss/file/remove?url='+file.response.data.url)
      .then((response)=>{
        console.log('远程文件删除')
        this.borrower.borrowerAttachList=this.borrower.borrowerAttachList.filter(function(item){
